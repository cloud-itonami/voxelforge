(ns voxelforge.app
  "voxelforge appview — reagent + re-frame, view built from
  jp-go-dds (デジタル庁デザインシステム) hiccup.

  This is a faithful port of the previous SvelteKit scaffold
  (`svelte/src/routes/+page.svelte`, moved from the pre-extraction path
  `60-apps/etzhayyim-project-voxelforge/svelte/src/routes/+page.svelte`
  per `migration.edn`): a static info page describing this component's
  own metadata — title/project/name/kind, its declared route count and
  public routes, its declared runtime vars, whether xrpc is configured,
  and the source path of the page that renders all of this. The Svelte
  page never called the dispatcher or the kotoba/ reference slice — it
  was pure static markup describing the surface's own metadata, and this
  port adds no dispatcher/xrpc functionality that was not already there.

  The Svelte scaffold's `routeCount`/`routes`/`vars` fields were stale
  empty literals (`0` / `[]` / `[]`); this port corrects them from the
  component's actual `wrangler.jsonc` (`routes[].pattern` and the `vars`
  map keys), same pattern as other ported scaffolds in this fleet
  (e.g. `cloud-itonami/app-tia`'s `tia-mcp-component/cljs`).

  Backend TypeScript is unaffected by this migration: `../src/app.ts`
  (the thin edge Worker) and `../src/dispatcher.ts` (the HMAC-signing
  forwarder to bpmn-dispatcher) are untouched. The one other TypeScript
  file that lived under the deleted `svelte/` tree —
  `svelte/src/routes/xrpc/[...path]/+server.ts`, a SvelteKit server
  route that proxied XRPC calls to AGENTGATEWAY_MCP_ROUTER_URL — was
  backend/XRPC logic, not frontend markup, so it was moved rather than
  deleted; see `../src/xrpc-agentgateway-proxy.ts` for its unmodified
  body and a note on why it is not wired to anything right now."
...
;; svelte/src/routes/+page.svelte to here. :route-count and :routes are
;; corrected from wrangler.jsonc (the Svelte scaffold had a stale 0 /
;; empty array there — `routeCount: 0, routes: []` — even though
;; wrangler.jsonc already declared one route). :vars is corrected the
;; same way, sorted alphabetically (the Svelte scaffold had `vars: []`).

(def default-db
  {:title "Ai etzhayyim Project Voxelforge"
   :project "etzhayyim-project-voxelforge"
   :name "etzhayyim-project-voxelforge"
   :kind "cloudflare surface"
   :route-count 1
   :routes ["voxelforge.etzhayyim.com/*"]
   :vars ["AGENTGATEWAY_MCP_ROUTER_URL" "APP_FRAMEWORK" "AUTHN_URL" "PDS_URL"
          "VOXELFORGE_ACTOR_DID" "VOXELFORGE_VERSION"]
   :xrpc? true
   :relative-path "cljs/src/voxelforge/app.cljs"})
